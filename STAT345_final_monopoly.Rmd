...
#position[[1]]
```
#see lines 495-564 for updated function
...
#Updated Moving Functions: (05-03-2021) 
#(Note: You will need to run chance_or_community_chest() prior to running these functions.)
```{r}
...
```{r}
#2) moving_doubles() is used when multiple dice rolls need to be considered. This stores player's last location and moves the player from their last known location.
...
#NEW SECTION: CHANCE AND COMMUNITY CHEST (05-02-2021)

#Landing on Chance or Community Chest:
```{r}
chance_or_community_chest <- function(playerID)
{
  ...
  if(player_position %in% chance_locations)
  {
    ...
    print(paste0("Your card reads: ", specific_action$Objective))
    location <- player_position
    
    if(specific_action$Places != "")
    {
      problem_cards <- c("Nearest Utility", "Nearest Railroad", "-3")
...
action_card_movement <- function(playerID, player_location, card_text)
{
  ...
  if((card_text != "Nearest Utility") && (card_text != "Nearest Railroad") && (card_text != "-3"))
  {
